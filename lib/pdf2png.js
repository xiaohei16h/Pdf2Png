...
function getImage(callback, options, imageFilepath, resp, i){
	exec("gs -dUseCropBox -dQUIET -dPARANOIDSAFER -dBATCH -dNOPAUSE -dNOPROMPT -sDEVICE=png16m -dTextAlphaBits=4 -dGraphicsAlphaBits=4 -r" + options.quality + " -dFirstPage="+i+" -dLastPage="+i+" -sOutputFile=" + imageFilepath + " " + '"' + resp.data + '"', function (error, stdout, stderr) {
		// Remove temp files
		resp.clean();
	...
	filesource.getDataPath(filepathOrData, function(resp){
		...
		getPageCount(function(resp2) {
			...
			if(!resp2.success) {
				callback(resp2);
				return;
			} else if (resp2.data == 0) {
				callback({success: false, error: 'page count is 0'});
				return;
			}
